...
import rx.schedulers.Scheduler.Worker;
...
public abstract class AbstractSchedulerConcurrencyTests extends AbstractSchedulerTests {
    ...
    public final void testUnSubscribeForScheduler() throws InterruptedException {
        ...
        Observable.interval(50, TimeUnit.MILLISECONDS)
                ...
                .subscribe(new AbstractSubscriber<Long>() {
                    @Override
    ...
    public void testUnsubscribeRecursiveScheduleFromOutside() throws InterruptedException {
        ...
        try {
            inner.schedule(() -> inner.schedule(new Runnable() {
        ...
        try {
            Observable<Integer> obs = Observable.create(new OnSubscribe<Integer>() {
                ...
                public void accept(final Subscriber<? super Integer> child) {
                    DisposableSubscriber<? super Integer> cs = DisposableSubscriber.from(child);
                    inner.schedule(new Runnable() {
                        @Override
                        public void run() {
                            cs.onNext(42);
            ...
            AbstractSubscriber<Integer> s = new AbstractSubscriber<Integer>() {
                @Override
                public void onComplete() {
                    System.out.println("Completed");
                    completed.set(true);
