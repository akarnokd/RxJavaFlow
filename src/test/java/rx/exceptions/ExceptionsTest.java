...
public class ExceptionsTest {
    ...
    public void testOnErrorNotImplementedIsThrown() {
        Observable.just(1, 2, 3).subscribe(t1 -> { throw new RuntimeException("hello"); });
    }
    ...
    public void testThreadDeathIsThrown() {
        Observable.just(1).subscribe(new AbstractSubscriber<Integer>() {
    ...
    public void testOnErrorExceptionIsThrown() {
        try {
            Observable.error(new IllegalArgumentException("original exception")).subscribe(new AbstractSubscriber<Object>() {
                @Override
                public void onComplete() {
