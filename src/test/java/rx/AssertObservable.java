...
public final class AssertObservable {
    ...
    public static <T> Observable<Void> assertObservableEquals(final String message, Observable<T> expected, Observable<T> actual) {
        ...
        Observable<Void> outcomeObservable = Observable.zip(expected.materialize(), actual.materialize(), zipFunction).reduce(accumulator).map(new Function<Notification<String>, Notification<Void>>() {
            @Override
