...
public final class PublishSubject<T> extends Subject<T, T> {
        ...
        OnSubscribe<T> onSubscribe = subscriber -> {
            ...
            if (!psm.add(subscriber)) {
                ...
                if (nl.isError(v)) {
                    ...
                    subscriber.onError(new IllegalStateException("PublishSubject has a value instead of terminal event: " + v));
                }
            } else {
                ds.add(Disposable.from(() -> psm.remove(subscriber)));
            }
        };
        return new PublishSubject<>(onSubscribe, psm);
    }
    ...
    protected PublishSubject(OnSubscribe<T> onSubscribe, SubjectSubscriberManager<T> psm) {
        super(onSubscribe);
        this.psm = psm;
    }
