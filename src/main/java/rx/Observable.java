...
/**
 * The Observable class that implements the Reactive Pattern via the java Flow API and Reactive-Streams specification.
 * <p>
...
public class Observable<T> implements Publisher<T> {
     ...
     */
    @FunctionalInterface
    public interface Transformer<T, R> extends Function<Observable<T>, Observable<R>> {
        // cover for generics insanity
    }
     ...
     * {@link Subscriber#onComplete() onComplete()} methods appropriately.
     * <p>
     * A well-formed Observable must invoke either the Subscriber's {@code onComplete()} method exactly once or
     ...
     *            {@code onError}, and {@code onComplete()} methods as appropriate
     ...
     * <p>
     * In other words, this allows chaining Subscribers together on an Observable for acting on the values within
     * the Observable.
     * <p> {@code
    ...
    /**
     * Subscribes the given subscription with this Observable, wrapping it into a Disposable
    ...
    /**
     * Subscribes the given subscription unsafely with this Observable, wrapping it into a Disposable
     * resource if necessary, and returning a Disposable reference to it. 
     * @param subscriber the subscriber to subscribe and wrap if itself doesn't implement Disposable.
     * @return the disposable that allows cancelling the subscription/subscriber from outside.
     * @since 2.0
    ...
    // -----------------------------------------------------------
    /**
     * Wraps a general Publisher instance into a Observable instance (if not already a Observable).
     * @param publisher the publisher to wrap
     * @return the Observable instance
     * @since 2.0
     */
    public static <T> Observable<T> from(Publisher<? extends T> publisher) {
        Objects.requireNonNull(publisher);
        if (publisher instanceof Observable) {
            @SuppressWarnings("unchecked")
            Observable<T> fo = (Observable<T>) publisher;
            return fo;
        }
    ...
    public static <T> Observable<T> just(T value) {
        return ScalarSynchronousObservable.create(Conformance.itemNonNull(value));
    }
    ...
    static final Observable<Object> EMPTY = create(new OnSubscribeEmpty<>());
    /**
     * Returns an Observable that emits no items to the {@link Subscriber} and immediately invokes its
     ...
     * @return an Observable that emits no items to the {@link Subscriber} but immediately invokes the
    ...
    /** A common, stateless never Observable. */
    static final Observable<Object> NEVER = create(new OnSubscribeNever<>());
    /**
     ...
     * <p>
     * This method returns an Observable that will invoke a subscribing {@link Subscriber}'s
     ...
     * @return the source Observable modified so that its {@link Subscriber}s are notified on the specified
     *         {@link Scheduler}
    ...
    /**
     * Asynchronously subscribes Subscribers to this Observable on the specified {@link Scheduler}.
     * <p>
     ...
     */
    public final <R> Observable<R> flatMap(Function<? super T, ? extends Observable<? extends R>> func) {
     ...
     */
    public final <R> Observable<R> flatMap(Function<? super T, ? extends Observable<? extends R>> func, int maxConcurrent) {
    ...
    /**
     * Flattens an Observable that emits Observables into one Observable, in a way that allows an Subscriber to
     ...
     */
    @SafeVarargs
    public final static <T> Observable<T> merge(int maxConcurrent, Observable<? extends T>... sequences) {
        return merge(from(sequences), maxConcurrent);
    }
    /**
     * Flattens an Observable that emits Observables into one Observable, in a way that allows an Subscriber to
     ...
     * <p>
     * Normally, an Observable that returns multiple items will do so by invoking its {@link Subscriber}'s
     * {@link Subscriber#onNext onNext} method for each such item. You can change this behavior, instructing the
     * Observable to compose a list of all of these items and then to invoke the Subscriber's {@code onNext}
    ...
    /**
     * Modifies the source Observable so that it invokes an action when it calls {@code onComplete()}.
     * <p>
     ...
     * 
     * @param onComplete()
     *            the action to invoke when the source Observable calls {@code onComplete()}
     * @return the source Observable with the side-effecting behavior applied
     * @see <a href="http://reactivex.io/documentation/operators/do.html">ReactiveX operators documentation: Do</a>
     */
    public final Observable<T> doOnComplete(final Runnable onComplete) {
        return lift(new OperatorDoOnEach<T>(v -> { }, e -> { }, onComplete));
    }
     ...
     * @see <a href="http://reactivex.io/documentation/operators/do.html">ReactiveX operators documentation: Do</a>
     */
    public final Observable<T> doOnEach(final Consumer<Notification<? super T>> onNotification) {
    ...
    /**
     * Modifies the source Observable so that it notifies an Subscriber for each item it emits.
     * <p>
     * <img width="640" height="310" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/doOnEach.png" alt="">
     * <dl>
     *  <dt><b>Scheduler:</b></dt>
     ...
     * @see <a href="http://reactivex.io/documentation/operators/do.html">ReactiveX operators documentation: Do</a>
     */
    public final Observable<T> doOnEach(Consumer<? super T> onNext,
     ...
     * @see <a href="http://reactivex.io/documentation/operators/do.html">ReactiveX operators documentation: Do</a>
     */
    public final Observable<T> doOnError(final Consumer<Throwable> onError) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/do.html">ReactiveX operators documentation: Do</a>
     */
    public final Observable<T> doOnNext(final Consumer<? super T> onNext) {
    ...
    /**
     * Returns an Observable that invokes an {@link Subscriber}'s {@link Subscriber#onError onError} method when the
     ...
     * @param <T>
     *            the type of the items (ostensibly) emitted by the Observable
     * @return an Observable that invokes the {@link Subscriber}'s {@link Subscriber#onError onError} method when
     ...
     * @see <a href="http://reactivex.io/documentation/operators/first.html">ReactiveX operators documentation: First</a>
     */
    public final Observable<T> single(Predicate<? super T> predicate) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/first.html">ReactiveX operators documentation: First</a>
     */
    public final Observable<T> singleOrDefault(T defaultValue, Predicate<? super T> predicate) {
     ...
     */
    public final Observable<T> reduce(BinaryOperator<T> accumulator) {
        /*
     ...
     */
    public final Observable<T> scan(BinaryOperator<T> accumulator) {
    ...
    /**
     * Returns a Observable which returns the value or error produced by the given CompletableFuture
     * asynchronously.
     * @param future the CompletableFuture to listen to
     * @return the Observable instance
     * @since 2.0
     */
    public static <T> Observable<T> from(CompletableFuture<T> future) {
     ...
     */
    @SuppressWarnings("unchecked")
    public static final <T1, T2, R> Observable<R> combineLatest(Observable<? extends T1> o1, Observable<? extends T2> o2, BiFunction<? super T1, ? super T2, ? extends R> combineFunction) {
    ...
    /**
     * Returns an Observable that calls an Observable factory to create an Observable for each new Subscriber
     ...
     * <p>
     * The defer Subscriber allows you to defer or delay emitting items from an Observable until such time as an
     * Subscriber subscribes to the Observable. This allows an {@link Subscriber} to easily obtain updates or a
     ...
     *            the Observable factory function to invoke for each {@link Subscriber} that subscribes to the
     *            resulting Observable
     * @param <T>
     *            the type of the items emitted by the Observable
     * @return an Observable whose {@link Subscriber}s' subscriptions trigger an invocation of the given
     ...
     */
    public final static <T> Observable<T> defer(Supplier<Observable<T>> observableFactory) {
    ...
    /**
     * Flattens two Observables into one Observable, in a way that allows an Subscriber to receive all
    ...
    /**
     * Flattens three Observables into one Observable, in a way that allows an Subscriber to receive all
     ...
     * Observables notify of an error via {@link Subscriber#onError onError}, {@code mergeDelayError} will refrain
    ...
    /**
     * Flattens four Observables into one Observable, in a way that allows an Subscriber to receive all
     * successfully emitted items from all of the source Observables without being interrupted by an error
     * notification from one of them.
     * <p>
     * This behaves like {@link #merge(Observable, Observable, Observable, Observable)} except that if any of
     * the merged Observables notify of an error via {@link Subscriber#onError onError}, {@code mergeDelayError}
    ...
    /**
     * Flattens five Observables into one Observable, in a way that allows an Subscriber to receive all
     * successfully emitted items from all of the source Observables without being interrupted by an error
     * notification from one of them.
     * <p>
     * This behaves like {@link #merge(Observable, Observable, Observable, Observable, Observable)} except that
     * if any of the merged Observables notify of an error via {@link Subscriber#onError onError},
    ...
    /**
     * Flattens six Observables into one Observable, in a way that allows an Subscriber to receive all
     * successfully emitted items from all of the source Observables without being interrupted by an error
     * notification from one of them.
     * <p>
     * This behaves like {@link #merge(Observable, Observable, Observable, Observable, Observable, Observable)}
     * except that if any of the merged Observables notify of an error via {@link Subscriber#onError onError},
     * {@code mergeDelayError} will refrain from propagating that error notification until all of the merged
     * Observables have finished emitting items.
     * <p>
     * <img width="640" height="380" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/mergeDelayError.png" alt="">
    ...
    /**
     * Flattens seven Observables into one Observable, in a way that allows an Subscriber to receive all
     * successfully emitted items from all of the source Observables without being interrupted by an error
     * notification from one of them.
     * <p>
     * This behaves like
    ...
    /**
     * Flattens eight Observables into one Observable, in a way that allows an Subscriber to receive all
     * successfully emitted items from all of the source Observables without being interrupted by an error
     * notification from one of them.
     * <p>
    ...
    /**
     * Flattens nine Observables into one Observable, in a way that allows an Subscriber to receive all
     * successfully emitted items from all of the source Observables without being interrupted by an error
     * notification from one of them.
     * <p>
     ...
     * @see <a href="http://reactivex.io/documentation/operators/sequenceequal.html">ReactiveX operators documentation: SequenceEqual</a>
     */
    public final static <T> Observable<Boolean> sequenceEqual(Observable<? extends T> first, Observable<? extends T> second, BiPredicate<? super T, ? super T> equality) {
    ...
    public final static <T, Resource> Observable<T> using(
            ...
            final Function<? super Resource, ? extends Observable<? extends T>> observableFactory,
    ...
    public final static <R> Observable<R> zip(Observable<? extends Observable<?>> ws, final FuncN<? extends R> zipFunction) {
        return ws.toList().map(new Function<List<? extends Observable<?>>, Observable<?>[]>() {
     ...
     * <p>
     * The resulting {@code Observable<R>} returned from {@code zip} will invoke {@link Subscriber#onNext onNext}
     ...
     * @see <a href="http://reactivex.io/documentation/operators/zip.html">ReactiveX operators documentation: Zip</a>
     */
    public final static <T1, T2, R> Observable<R> zip(Observable<? extends T1> o1, Observable<? extends T2> o2, final BiFunction<? super T1, ? super T2, ? extends R> zipFunction) {
     ...
     */
    public final Observable<Boolean> all(Prdicate<? super T> predicate) {
     ...
     *            a {@link Supplier} that produces an Observable that governs the boundary between buffers.
     ...
     */
    public final <TClosing> Observable<List<T>> buffer(Supplier<? extends Observable<? extends TClosing>> bufferClosingSelector) {
     ...
     * @param bufferClosingSelector
     *            the {@link Function} that is used to produce an Observable for every buffer created. When this
     ...
     * @see <a href="http://reactivex.io/documentation/operators/buffer.html">ReactiveX operators documentation: Buffer</a>
     */
    public final <TOpening, TClosing> Observable<List<T>> buffer(Observable<? extends TOpening> bufferOpenings, Function<? super TOpening, ? extends Observable<? extends TClosing>> bufferClosingSelector) {
     ...
     * Subscriber so be careful not to use this Subscriber on Observables that emit an infinite or very large number
     ...
     * @see <a href="http://reactivex.io/documentation/operators/reduce.html">ReactiveX operators documentation: Reduce</a>
     */
    public final <R> Observable<R> collect(Supplier<R> stateFactory, final Action2<R, ? super T> collector) {
        BiFunction<R, T, R> accumulator = new BiFunction<R, T, R>() {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/flatmap.html">ReactiveX operators documentation: FlatMap</a>
     */
    public final <R> Observable<R> concatMap(Function<? super T, ? extends Observable<? extends R>> func) {
    ...
    public final <U, V> Observable<T> delay(
            Supplier<? extends Observable<U>> subscriptionDelay,
     ...
     * @see <a href="http://reactivex.io/documentation/operators/delay.html">ReactiveX operators documentation: Delay</a>
     */
    public final <U> Observable<T> delay(Function<? super T, ? extends Observable<U>> itemDelay) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/delay.html">ReactiveX operators documentation: Delay</a>
     */
    public final <U> Observable<T> delaySubscription(Supplier<? extends Observable<U>> subscriptionDelay) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/distinct.html">ReactiveX operators documentation: Distinct</a>
     */
    public final <U> Observable<T> distinct(Function<? super T, ? extends U> keySelector) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/distinct.html">ReactiveX operators documentation: Distinct</a>
     */
    public final <U> Observable<T> distinctUntilChanged(Function<? super T, ? extends U> keySelector) {
     ...
     */
    @Beta
    public final Observable<T> doOnRequest(final LongConsumer onRequest) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/do.html">ReactiveX operators documentation: Do</a>
     */
    public final Observable<T> doOnSubscribe(final Runnable subscribe) {
        return lift(new OperatorDoOnSubscribe<T>(subscribe));
    }
    
    /**
     * Modifies the source Observable so that it invokes an action when it calls {@code onComplete()} or
     * {@code onError}.
     * <p>
     ...
     * 
     * @param onTerminate
     *            the action to invoke when the source Observable calls {@code onComplete()} or {@code onError}
     * @return the source Observable with the side-effecting behavior applied
     * @see <a href="http://reactivex.io/documentation/operators/do.html">ReactiveX operators documentation: Do</a>
     ...
     * @see <a href="http://reactivex.io/documentation/operators/do.html">ReactiveX operators documentation: Do</a>
     */
    public final Observable<T> doOnUnsubscribe(final Runnable unsubscribe) {
    ...
    /**
     * Registers an {@link Runnable} to be called when this Observable invokes either
     ...
     * 
     * @param action
     *            an {@link Runnable} to be invoked when the source Observable finishes
     * @return an Observable that emits the same items as the source Observable, then invokes the
     *         {@link Runnable}
     * @see <a href="http://reactivex.io/documentation/operators/do.html">ReactiveX operators documentation: Do</a>
     * @see #doOnTerminate(Runnable)
     */
    public final Observable<T> finallyDo(Runnable action) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/first.html">ReactiveX operators documentation: First</a>
     */
    public final Observable<T> first(Predicate<? super T> predicate) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/first.html">ReactiveX operators documentation: First</a>
     */
    public final Observable<T> firstOrDefault(T defaultValue, Predicate<? super T> predicate) {
     ...
     *            Observable
     * @param onComplete()
     *            a function that returns an Observable to merge for an onComplete() notification from the source
     *            Observable
    ...
    public final <R> Observable<R> flatMap(
            Function<? super T, ? extends Observable<? extends R>> onNext,
            Function<? super Throwable, ? extends Observable<? extends R>> onError,
            Supplier<? extends Observable<? extends R>> onComplete) {
        return merge(mapNotification(onNext, onError, onComplete));
    }
    /**
     * Returns an Observable that applies a function to each item emitted or notification raised by the source
     * Observable and then flattens the Observables returned from these functions and emits the resulting items, 
    ...
    public final <R> Observable<R> flatMap(
            Function<? super T, ? extends Observable<? extends R>> onNext,
            Function<? super Throwable, ? extends Observable<? extends R>> onError,
            Supplier<? extends Observable<? extends R>> onComplete, int maxConcurrent) {
        return merge(mapNotification(onNext, onError, onComplete), maxConcurrent);
    }
     ...
     * @see <a href="http://reactivex.io/documentation/operators/flatmap.html">ReactiveX operators documentation: FlatMap</a>
     */
    public final <U, R> Observable<R> flatMap(final Function<? super T, ? extends Observable<? extends U>> collectionSelector,
    ...
    @Beta
    public final <U, R> Observable<R> flatMap(final Function<? super T, ? extends Observable<? extends U>> collectionSelector,
            final BiFunction<? super T, ? super U, ? extends R> resultSelector, int maxConcurrent) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/flatmap.html">ReactiveX operators documentation: FlatMap</a>
     */
    public final <R> Observable<R> flatMapIterable(Function<? super T, ? extends Iterable<? extends R>> collectionSelector) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/flatmap.html">ReactiveX operators documentation: FlatMap</a>
     */
    public final <U, R> Observable<R> flatMapIterable(Function<? super T, ? extends Iterable<? extends U>> collectionSelector,
     ...
     */
    public final <K, R> Observable<GroupedObservable<K, R>> groupBy(final Function<? super T, ? extends K> keySelector, final Function<? super T, ? extends R> elementSelector) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/groupby.html">ReactiveX operators documentation: GroupBy</a>
     */
    public final <K> Observable<GroupedObservable<K, T>> groupBy(final Function<? super T, ? extends K> keySelector) {
     ...
     */
    public final <T2, D1, D2, R> Observable<R> groupJoin(Observable<T2> right, Function<? super T, ? extends Observable<D1>> leftDuration,
            Function<? super T2, ? extends Observable<D2>> rightDuration,
            BiFunction<? super T, ? super Observable<T2>, ? extends R> resultSelector) {
        return create(new OnSubscribeGroupJoin<T, T2, D1, D2, R>(this, right, leftDuration, rightDuration, resultSelector));
    }

    /**
     * Ignores all items emitted by the source Observable and only calls {@code onComplete()} or {@code onError}.
     * <p>
     ...
     * </dl>
     * 
     * @return an empty Observable that only calls {@code onComplete()} or {@code onError}, based on which one is
     ...
     * @see <a href="http://reactivex.io/documentation/operators/join.html">ReactiveX operators documentation: Join</a>
     */
    public final <TRight, TLeftDuration, TRightDuration, R> Observable<R> join(Observable<TRight> right, Function<T, Observable<TLeftDuration>> leftDurationSelector,
            Function<TRight, Observable<TRightDuration>> rightDurationSelector,
            BiFunction<T, TRight, R> resultSelector) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/last.html">ReactiveX operators documentation: Last</a>
     */
    public final Observable<T> last(Predicate<? super T> predicate) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/last.html">ReactiveX operators documentation: Last</a>
     */
    public final Observable<T> lastOrDefault(T defaultValue, Predicate<? super T> predicate) {
    ...
    private final <R> Observable<R> mapNotification(Function<? super T, ? extends R> onNext, Function<? super Throwable, ? extends R> onError, Supplier<? extends R> onComplete) {
        return lift(new OperatorMapNotification<T, R>(onNext, onError, onComplete));
    }
     ...
     */
    @Beta
    public final Observable<T> onBackpressureBuffer(long capacity, Runnable onOverflow) {
     ...
     * its {@link Subscriber}, the Observable invokes its Subscriber's {@code onError} method, and then quits
     ...
     * Subscriber's {@code onError} method, it will instead relinquish control to the Observable returned from
     * {@code resumeFunction}, which will invoke the Subscriber's {@link Subscriber#onNext onNext} method if it is
     * able to do so. In such a case, because no Observable necessarily invokes {@code onError}, the Subscriber
     ...
     */
    public final Observable<T> onErrorResumeNext(final Function<Throwable, ? extends Observable<? extends T>> resumeFunction) {
     ...
     * because no Observable necessarily invokes {@code onError}, the Subscriber may never know that an error
     * happened.
     * <p>
     * You can use this to prevent errors from propagating or to supply fallback data should errors be
     * encountered.
     ...
     * method, if the original Observable encounters an error, instead of invoking its Subscriber's
     ...
     * @see <a href="http://reactivex.io/documentation/operators/catch.html">ReactiveX operators documentation: Catch</a>
     */
    public final Observable<T> onErrorReturn(Function<Throwable, ? extends T> resumeFunction) {
     ...
     * to its {@link Subscriber}, the Observable invokes its Subscriber's {@code onError} method, and then quits
     ...
     * call {@code onComplete()} or {@code onError} on the child subscription. Otherwise, this Observable will
     ...
     * @see <a href="http://reactivex.io/documentation/operators/repeat.html">ReactiveX operators documentation: Repeat</a>
     */
    public final Observable<T> repeatWhen(final Function<? super Observable<? extends Void>, ? extends Observable<?>> notificationHandler, Scheduler scheduler) {
        Function<? super Observable<? extends Notification<?>>, ? extends Observable<?>> dematerializedNotificationHandler = new Function<Observable<? extends Notification<?>>, Observable<?>>() {
            @Override
     ...
     * @see <a href="http://reactivex.io/documentation/operators/repeat.html">ReactiveX operators documentation: Repeat</a>
     */
    public final Observable<T> repeatWhen(final Function<? super Observable<? extends Void>, ? extends Observable<?>> notificationHandler) {
        Function<? super Observable<? extends Notification<?>>, ? extends Observable<?>> dematerializedNotificationHandler = new Function<Observable<? extends Notification<?>>, Observable<?>>() {
            @Override
            public Observable<?> call(Observable<? extends Notification<?>> notifications) {
                return notificationHandler.call(notifications.map(new Function<Notification<?>, Void>() {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/replay.html">ReactiveX operators documentation: Replay</a>
     */
    public final <R> Observable<R> replay(Function<? super Observable<T>, ? extends Observable<R>> selector, final int bufferSize, final Scheduler scheduler) {
        return create(new OnSubscribeMulticastSelector<T, T, R>(this, new Supplier<Subject<T, T>>() {
            @Override
            public final Subject<T, T> call() {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/replay.html">ReactiveX operators documentation: Replay</a>
     */
    public final <R> Observable<R> replay(Function<? super Observable<T>, ? extends Observable<R>> selector, final Scheduler scheduler) {
        return create(new OnSubscribeMulticastSelector<T, T, R>(this, new Supplier<Subject<T, T>>() {
            @Override
            public final Subject<T, T> call() {
     ...
     * <p>
     * If the source Observable calls {@link Subscriber#onError}, this method will resubscribe to the source
     ...
     * @see <a href="http://reactivex.io/documentation/operators/retry.html">ReactiveX operators documentation: Retry</a>
     */
    public final Observable<T> retry(BiPredicate<Integer, Throwable> predicate) {
     ...
     * {@code onComplete()} or {@code onError} on the child subscription. Otherwise, this Observable will
     * resubscribe to the source Observable.    
     * <p>
     ...
     * @see <a href="http://reactivex.io/documentation/operators/retry.html">ReactiveX operators documentation: Retry</a>
     */
    public final Observable<T> retryWhen(final Function<? super Observable<? extends Throwable>, ? extends Observable<?>> notificationHandler) {
        Function<? super Observable<? extends Notification<?>>, ? extends Observable<?>> dematerializedNotificationHandler = new Function<Observable<? extends Notification<?>>, Observable<?>>() {
            @Override
            public Observable<?> call(Observable<? extends Notification<?>> notifications) {
     ...
     * {@code onComplete} or {@code onError} then {@code retry} will call {@code onComplete()} or {@code onError}
     ...
     * @see <a href="http://reactivex.io/documentation/operators/retry.html">ReactiveX operators documentation: Retry</a>
     */
    public final Observable<T> retryWhen(final Function<? super Observable<? extends Throwable>, ? extends Observable<?>> notificationHandler, Scheduler scheduler) {
        Function<? super Observable<? extends Notification<?>>, ? extends Observable<?>> dematerializedNotificationHandler = new Function<Observable<? extends Notification<?>>, Observable<?>>() {
            @Override
            public Observable<?> call(Observable<? extends Notification<?>> notifications) {
                return notificationHandler.call(notifications.map(new Function<Notification<?>, Throwable>() {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/flatmap.html">ReactiveX operators documentation: FlatMap</a>
     */
    public final <R> Observable<R> switchMap(Function<? super T, ? extends Observable<? extends R>> func) {
     ...
     * @see Observable#takeUntil(Function)
     */
    public final Observable<T> takeWhile(final Predicate<? super T> predicate) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/takeuntil.html">ReactiveX operators documentation: TakeUntil</a>
     * @see Observable#takeWhile(Function)
     * @since (if this graduates from Experimental/Beta to supported, replace this parenthetical with the release number)
     */
    @Experimental
    public final Observable<T> takeUntil(final Predicate<? super T> stopPredicate) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/timeout.html">ReactiveX operators documentation: Timeout</a>
     */
    public final <U, V> Observable<T> timeout(Supplier<? extends Observable<U>> firstTimeoutSelector, Function<? super T, ? extends Observable<V>> timeoutSelector) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/timeout.html">ReactiveX operators documentation: Timeout</a>
     */
    public final <U, V> Observable<T> timeout(Supplier<? extends Observable<U>> firstTimeoutSelector, Function<? super T, ? extends Observable<V>> timeoutSelector, Observable<? extends T> other) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/to.html">ReactiveX operators documentation: To</a>
     */
    public final <K, V> Observable<Map<K, V>> toMap(Function<? super T, ? extends K> keySelector, Function<? super T, ? extends V> valueSelector, Supplier<? extends Map<K, V>> mapFactory) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/to.html">ReactiveX operators documentation: To</a>
     */
    public final <K, V> Observable<Map<K, Collection<V>>> toMultimap(Function<? super T, ? extends K> keySelector, Function<? super T, ? extends V> valueSelector, Supplier<? extends Map<K, Collection<V>>> mapFactory) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/to.html">ReactiveX operators documentation: To</a>
     */
    public final <K, V> Observable<Map<K, Collection<V>>> toMultimap(Function<? super T, ? extends K> keySelector, Function<? super T, ? extends V> valueSelector, Supplier<? extends Map<K, Collection<V>>> mapFactory, Function<? super K, ? extends Collection<V>> collectionFactory) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/to.html">ReactiveX operators documentation: To</a>
     */
    public final Observable<List<T>> toSortedList(BiFunction<? super T, ? super T, Integer> sortFunction) {
     ...
     */
    @Experimental
    public final <U, R> Observable<R> withLatestFrom(Observable<? extends U> other, BiFunction<? super T, ? super U, ? extends R> resultSelector) {
     ...
     *            a {@link Supplier} that returns an {@code Observable} that governs the boundary between windows.
     ...
     */
    public final <TClosing> Observable<Observable<T>> window(Supplier<? extends Observable<? extends TClosing>> closingSelector) {
     ...
     * @param closingSelector
     *            a {@link Function} that produces an Observable for every window created. When this Observable
     ...
     * @see <a href="http://reactivex.io/documentation/operators/window.html">ReactiveX operators documentation: Window</a>
     */
    public final <TOpening, TClosing> Observable<Observable<T>> window(Observable<? extends TOpening> windowOpenings, Function<? super TOpening, ? extends Observable<? extends TClosing>> closingSelector) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/zip.html">ReactiveX operators documentation: Zip</a>
     */
    public final <T2, R> Observable<R> zipWith(Iterable<? extends T2> other, BiFunction<? super T, ? super T2, ? extends R> zipFunction) {
     ...
     * @see <a href="http://reactivex.io/documentation/operators/zip.html">ReactiveX operators documentation: Zip</a>
     */
    public final <T2, R> Observable<R> zipWith(Observable<? extends T2> other, BiFunction<? super T, ? super T2, ? extends R> zipFunction) {
