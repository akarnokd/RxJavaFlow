...
/**
 * Returns an Iterable that blocks until the Flowable emits another item, then returns that item.
 * <p>
...
public final class BlockingOperatorNext {
    ...
    /**
     * Returns an {@code Iterable} that blocks until the {@code Flowable} emits another item, then returns
     * that item.
     *
     * @param items
     *            the {@code Flowable} to observe
     * @return an {@code Iterable} that behaves like a blocking version of {@code items}
     */
    public static <T> Iterable<T> next(final Flowable<? extends T> items) {
    ...
    /* private */static final class NextIterator<T> implements Iterator<T> {
        ...
        private final Flowable<? extends T> items;
        private T next;
        ...
        private NextIterator(Flowable<? extends T> items, NextObserver<T> observer) {
            this.items = items;
    ...
    private static class NextObserver<T> extends AbstractDisposableSubscriber<Notification<? extends T>> {
        ...
        @Override
        public void internalOnNext(Notification<? extends T> args) {
